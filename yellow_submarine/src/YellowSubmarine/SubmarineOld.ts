...
import {SubmarineCameraOld} from "@/YellowSubmarine/SubmarineCameraOld";
import {GameOld} from "@/YellowSubmarine/GameOld";
import {WorldOld} from "@/YellowSubmarine/WorldOld"

export class SubmarineOld {
    ...
    private _submarineCamera: SubmarineCameraOld;

    constructor() {
        this._mesh = this.createMesh();
        this._submarineCamera = new SubmarineCameraOld(this);
        WorldOld.scene.onBeforeRenderObservable.add(() => {
            this.update(GameOld.engine.getDeltaTime() / 1000);
        });
        WorldOld.scene.actionManager.registerAction(
    ...
    private createMesh(): Mesh {
        const mesh = MeshBuilder.CreateBox("player", {width: 2, depth: 4}, WorldOld.scene);
